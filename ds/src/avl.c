                             ...
                             void *data, avl_params_ty *which_child)
{
	...
	side = 0 > cmp_result;
	if (NULL == NodeGetChildren(node, side))
		{
			*which_child = side;
			return (node);
		}
	return (FindPosition(avl, NodeGetChildren(node, side),
		                    data, which_child));
}
static node_ty *ReBalancing(avl_ty *avl, node_ty *node, 
                          void *data)
{
	...
	node->children[side] = ReBalancing(avl, NodeGetChildren(node, side),
		              data);
