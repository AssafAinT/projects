...
static int CompareData(void *data1, void *data2 , void *params);

static int IsMatchUid(const void *uid_to_remove, void *current_task);
                        ...
                        void *cleanup_param, size_t time_interval)
{
	...
	PQueueEnqueue(GET_SCHEDULER(scheduler), task_to_send);
	uid_to_ret = GetUid(task_to_send);
	return (uid_to_ret);
}
...
static int IsMatchUid(const void *uid_p, void *task)
{
