...
static void PTprint_count(void)
{
    printf("s_count: %d\n",PTs_count);
}

func_ptr VtblPublicTransport[2] = {DtorPublicTransport, PTdisplay};
...
{
    this->VPTR = VtblPublicTransport;
    this->m_license_plate = ++PTs_count;
...
func_ptr VtblMinibus[3] = {DtorMiniBus, MBdisplay, Wash};
...
void CtorMiniBus(struct Minibus *this) 
{
    ...
    this->public_transport.VPTR = VtblMinibus;
    printf("Minibus::Ctor()\n");
}
...
func_ptr VtblTaxi[2] = {DtorTaxi, Taxidisplay};
void CtorTaxi(struct Taxi *this) 
{
    CtorPublicTransport(&this->public_transport);
    this->public_transport.VPTR = VtblTaxi;
    printf("Taxi::Ctor()\n");
}
...
func_ptr SpecialTaxi_VTablei[2] = {SpecialTaxiDtor, SpecialTaxiDisplay};

void taxi_display(struct Taxi s)
{
    ((func_ptr*)(s.public_transport.VPTR))[1](&s);
...
{
    ((func_ptr*)(m->public_transport.VPTR))[2](m, 3);
}
...
{
    ((func_ptr*)(this->public_transport.VPTR))[0](this);
    free(this);
}
...
{
    ((func_ptr*)(this->m_pt1->VPTR))[0](this->m_pt1);
    free(this->m_pt1);
    ((func_ptr*)(this->m_pt2->VPTR))[0](this->m_pt2);
    free(this->m_pt2);
    DtorTaxi(&(this->m_t));
...
{
    ((func_ptr*)(this->m_pt1->VPTR))[1](this->m_pt1);
    ((func_ptr*)(this->m_pt2->VPTR))[1](this->m_pt2);
...
func_ptr PublicConvoy_VTable[2] = {PublicConvoyDtor, PublicConvoyDisplay};


void PublicConvoyCtor(struct PublicConvoy *this)
{
    CtorPublicTransport(&(this->public_transport));
    this->public_transport.VPTR = PublicConvoy_VTable;
...
int main(void)
{
    ...
    ((func_ptr *)ret_func.VPTR)[1](&ret_func);
    ...
    for (int i = 0; i < 3; ++i) 
    {
        ((func_ptr *)(array[i]->VPTR))[1](array[i]);
    }

    for (int i = 0; i < 3; ++i) 
    {
        ((func_ptr *)(array[i]->VPTR))[0](array[i]);
        free(array[i]);
    }
    ...
    ((func_ptr *)m_arr2.public_transport.VPTR)[0](&m_arr2);

    struct Taxi m_taxi;
    CtorTaxi(&m_taxi);
    struct PublicTransport public_taxi;
    PublicTransportCctor(&public_taxi, &m_taxi.public_transport);
    ((func_ptr *)m_taxi.public_transport.VPTR)[0](&m_taxi);
